...
export function createStudentsSDK(supabase, eventIds) {
    return {
        ...
        async search(params) {
            ...
            if (params.name) {
                query = query.or(`first_name.ilike.%${params.name}%,last_name.ilike.%${params.name}%`, { referencedTable: "students" });
            }
            if (params.email) {
        ...
        async getTicketOrder(studentId, eventId) {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
                .from("ticket_orders")
        ...
        async getAvailableTickets(studentId, eventId) {
            assertEventAllowed(eventId, eventIds);
        ...
        async transferToTeam(studentEventId, teamId) {
        ...
        async transferToOrg(studentEventId, orgId) {
            const { data, error } = await supabase
                .from("student_events")
                .update({
        ...
        async removeFromTeam(studentEventId) {
            const { data, error } = await supabase
                .from("student_events")
                .update({ team_id: null })
                .eq("student_event_id", studentEventId)
        ...
        async removeFromEvent(studentEventId) {
