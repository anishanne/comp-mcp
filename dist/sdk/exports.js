...
export function createExportsSDK(supabase, eventIds) {
    return {
        /** Export student list */
        async students(eventId, format = "json") {
        ...
        async teams(eventId, format = "json") {
            assertEventAllowed(eventId, eventIds);
                ...
                for (const team of teams || []) {
                    ...
                    for (const m of members || []) {
                        rows.push({
                            ...
                            org_id: team.org_id,
                            member_name: `${m.person?.first_name || ""} ${m.person?.last_name || ""}`.trim(),
                            member_email: m.person?.email,
                        });
                    }
                }
                return toCSV(rows);
        ...
        async orgs(eventId, format = "json") {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
                .from("org_events")
        ...
        async ticketOrders(eventId, format = "json") {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
                .from("ticket_orders")
        ...
        async refundRequests(eventId, format = "json") {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
