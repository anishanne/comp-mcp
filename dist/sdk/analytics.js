...
export function createAnalyticsSDK(supabase, eventIds) {
    return {
        ...
        async teamSizeDistribution(eventId) {
            assertEventAllowed(eventId, eventIds);
        ...
        async gradeSummary(eventId) {
            ...
            for (const test of tests || []) {
                ...
                if (gErr)
                    throw gErr;
                const scores = (graded || []).map((g) => g.score).filter((s) => s != null);
        ...
        async registrationTimeline(eventId) {
            assertEventAllowed(eventId, eventIds);
        ...
        async studentsWithoutTeam(eventId) {
            assertEventAllowed(eventId, eventIds);
            const { count, error } = await supabase
                .from("student_events")
                .select("*", { count: "exact", head: true })
         ...
         */
        async studentTestMatrix(eventId) {
            assertEventAllowed(eventId, eventIds);
            ...
            if (tErr)
                throw tErr;
            const testIds = (allTests || []).map((t) => t.test_id);
            if (testIds.length === 0)
                return { tests: [], rows: [] };
            const [{ data: answers, error: aErr }, { data: takers, error: tkErr }, { data: students, error: sErr },] = await Promise.all([
                supabase
                    .from("graded_test_answers")
            ...
            if (teamIds.length > 0) {
                ...
                if (tmErr)
                    throw tmErr;
                teamMap = new Map((teams || []).map((t) => [t.team_id, t.team_name]));
            }
        ...
        async teamTestMatrix(eventId) {
            ...
            if (testIds.length === 0)
                return { tests: [], rows: [] };
            const [{ data: answers, error: aErr }, { data: takers, error: tkErr }, { data: teams, error: tmErr },] = await Promise.all([
                supabase
                    .from("graded_test_answers")
                    .select("test_id, test_taker_id, score:points")
                    .in("test_id", testIds),
            ...
            for (const ans of answers || []) {
                ...
                if (tid == null)
                    continue;
                if (!teamScores.has(tid))
                    teamScores.set(tid, new Map());
                const m = teamScores.get(tid);
                m.set(ans.test_id, (m.get(ans.test_id) || 0) + (ans.score || 0));
            }
            const orgIds = [
                ...new Set((teams || []).map((t) => t.org_id).filter((x) => x != null)),
            ];
            let orgMap = new Map();
            if (orgIds.length > 0) {
                const { data: orgs, error: oErr } = await supabase
                ...
                if (oErr)
                    throw oErr;
                orgMap = new Map((orgs || []).map((o) => [o.org_id, o.name]));
            }
        ...
        async customFieldSummary(eventId, table = "students") {
            assertEventAllowed(eventId, eventIds);
