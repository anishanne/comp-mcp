 ...
 */
async function loadScanScores(supabase, testId, problemMap) {
    const byTakerFrontId = new Map();
    const takerFrontIds = new Set();
    ...
    const scanToTaker = new Map();
    for (const s of scans) {
        if (s.scan_id == null || s.taker_id == null)
            continue;
    ...
    for (const { scanId, tpid, rows } of grouped.values()) {
        ...
        if (override) {
            ...
            const distinct = [...new Set(rows.map((r) => r.grade).filter((g) => g != null))];
            if (distinct.length === 0)
                finalGrade = null;
            else if (distinct.length === 1)
                finalGrade = distinct[0];
            else
                finalGrade = "conflict";
        }
        let state;
        ...
        const lastEditedAt = rows
            ...
            .sort()
            .pop() ?? null;
        if (!byTakerFrontId.has(taker))
            byTakerFrontId.set(taker, []);
...
async function loadOnlineScores(supabase, testId, problemMap) {
    const out = new Map();
    const { data, error } = await supabase
        .from("graded_test_answers")
        .select("test_taker_id, test_problem_id, score:points, correct, answer_latex, last_edited_time")
        .eq("test_id", testId);
    if (error)
        throw error;
    for (const r of data || []) {
        if (r.test_taker_id == null || r.test_problem_id == null)
            continue;
        const tp = problemMap.get(r.test_problem_id);
        let state;
        if (r.answer_latex == null || r.answer_latex === "")
            state = "unanswered";
            ...
            state = "ungraded";
        if (!out.has(r.test_taker_id))
            out.set(r.test_taker_id, []);
...
async function loadManualScores(supabase, testId, problemMap) {
    const out = new Map();
    const { data, error } = await supabase
        .from("manual_grades")
    ...
    if (error)
        throw error;
    for (const r of data || []) {
        if (r.team_id == null || r.test_problem_id == null)
            continue;
        const tp = problemMap.get(r.test_problem_id);
        let state;
        let points = 0;
 ...
 */
async function buildEntityIndex(supabase, testId, eventId, isTeam, onlineTakerIds, manualTeamIds, scanTakerFrontIds) {
    const takerKey = new Map();
    const teamKey = new Map();
    const scanKey = new Map();
    ...
    const teamIds = new Set(manualTeamIds);
    ...
    const ensureEntity = (key, init) => {
        if (!entities.has(key))
            entities.set(key, init());
    ...
    for (const [takerId, info] of takerInfo) {
        ...
        if (e.test_taker_id == null)
            e.test_taker_id = takerId;
    ...
    for (const teamId of manualTeamIds) {
        ...
        if (e.front_id == null)
            e.front_id = teamFrontId.get(teamId) ?? null;
        if (!e.display_name)
            e.display_name = teamName.get(teamId) ?? null;
 ...
 */
function applyProblemScore(entity, ps) {
    const existing = entity.perProblem[ps.test_problem_id];
    if (existing) {
        // Source priority: scan > manual > online (paper / authoritative manual wins).
        const priority = { scan: 3, manual: 2, online: 1 };
        if (priority[ps.source] <= priority[existing.source])
            return;
...
export async function resolveTestScores(supabase, testId) {
    ...
    const sources = [];
    if (onlineByTaker.size > 0)
        sources.push("online");
    if (scanByFront.byTakerFrontId.size > 0)
        sources.push("scan");
    if (manualByTeam.size > 0)
        sources.push("manual");
    const { takerKey, teamKey, scanKey, entities } = await buildEntityIndex(supabase, testId, testInfo.event_id, !!testInfo.is_team, [...onlineByTaker.keys()], [...manualByTeam.keys()], scanByFront.takerFrontIds);
    ...
    const list = [...entities.values()].sort((a, b) => {
        ...
        if (a.lastSubmittedAt)
            return -1;
        if (b.lastSubmittedAt)
            return 1;
        return 0;
    });
    return {
