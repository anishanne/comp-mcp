...
export function createEventsSDK(supabase, eventIds) {
    return {
        ...
        async getOrganizations(eventId) {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
                .from("org_events")
                .select(`
        ...
        async getIndependentTeams(eventId) {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
                .from("teams")
                .select("*")
        ...
        async getCustomFields(eventId, table = "students") {
            assertEventAllowed(eventId, eventIds);
            const { data, error } = await supabase
