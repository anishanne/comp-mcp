...
export function createTicketsSDK(supabase, eventIds) {
    return {
        ...
        async listRefundRequests(eventId, status) {
            assertEventAllowed(eventId, eventIds);
            let query = supabase
         ...
         */
        async approveRefund(refundId, quantity, responseMessage) {
        ...
        async denyRefund(refundId, responseMessage) {
            const { data: refund, error: refundError } = await supabase
                .from("refund_requests")
                .select("*, ticket:ticket_orders(*)")
                .eq("id", refundId)
